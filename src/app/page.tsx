...
export default function Home() {
  const sections = [
    { len: 3, label: "3 Letters", items: "1,800+ words", color: "from-blue-600 to-indigo-700" },
    { len: 4, label: "4 Letters", items: "2,100+ words", color: "from-indigo-600 to-violet-700" },
    { len: 5, label: "5 Letters", items: "2,400+ words", color: "from-blue-600 to-cyan-700" },
    { len: 6, label: "6 Letters", items: "2,000+ words", color: "from-violet-600 to-purple-700" },
    { len: 7, label: "7 Letters", items: "1,700+ words", color: "from-purple-600 to-pink-700" }
  ];
    ...
    <div className="pt-40 pb-32 px-4 max-w-7xl mx-auto overflow-hidden">
      ...
      <section className="text-center mb-32 relative">
        ...
        <motion.div
          ...
          className="inline-flex items-center gap-2 bg-blue-50 dark:bg-blue-900/20 px-5 py-2 rounded-full text-blue-700 dark:text-blue-400 text-xs font-black mb-10 border border-blue-200 dark:border-blue-800"
        >
        ...
        <h1 className="text-6xl md:text-8xl font-black text-slate-900 dark:text-white mb-8 tracking-tighter leading-[0.9]">
          ...
          <span className="text-slate-800 dark:text-white/90">Perfect Word.</span>
        </h1>
        ...
        <div className="flex flex-wrap justify-center gap-6">
          ...
          <Link href="/about-us" className="bg-white dark:bg-slate-900 border-2 border-slate-200 dark:border-slate-800 text-slate-800 dark:text-slate-200 px-10 py-5 rounded-[2rem] font-black text-xl transition-all flex items-center gap-3 hover:bg-slate-100 dark:hover:bg-slate-800/50 shadow-sm">
            Our Mission
      ...
      <div className="flex items-center gap-4 mb-16">
        ...
        <h2 className="text-xs font-black text-slate-500 dark:text-slate-400 uppercase tracking-[0.4em]">Archival Segments</h2>
        <div className="h-px flex-1 bg-slate-300 dark:bg-slate-800" />
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6 mb-40">
        {sections.map((s, i) => (
          <Link key={s.len} href={`/list/${s.len}`} className="group relative p-10 bg-white dark:bg-slate-900 border-2 border-slate-150 dark:border-slate-800 rounded-[2.5rem] hover:border-blue-600 hover:shadow-2xl transition-all overflow-hidden">
            <div className="absolute top-0 right-0 p-8 opacity-[0.03] group-hover:scale-125 transition-transform duration-500 scale-100">
              <Search className="w-32 h-32" />
            </div>

            <div className={`w-14 h-14 bg-gradient-to-br ${s.color} rounded-2xl flex items-center justify-center text-white mb-8 shadow-lg relative z-10`}>
      ...
      {/* Features Showcase */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-12 py-24 border-y-2 border-slate-200 dark:border-slate-800">
        {[
